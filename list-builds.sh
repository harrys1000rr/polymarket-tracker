#!/bin/bash
TOKEN="nf-eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..TyeC3JLagjhE5H_C-jpwKmFAXMw1PGVYPPwSF1d2Oo0"

echo "Backend builds:"
curl -s "https://api.northflank.com/v1/projects/polymarketsim/services/backend/builds" \
  -H "Authorization: Bearer $TOKEN" | jq '.data.builds[:3] | .[] | {id, status, createdAt}'

echo ""
echo "Backend current status:"
curl -s "https://api.northflank.com/v1/projects/polymarketsim/services/backend" \
  -H "Authorization: Bearer $TOKEN" | jq '.data.status'
